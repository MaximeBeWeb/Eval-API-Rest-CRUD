...
exports.getDataById = (request, response) => {
    ...
    fs.readFile(road, (err, data) => {
        ...
        if (err) {
            ...
            const dataById = manipData.fromages.find(
...
exports.getDataByName = (request, response) => {
    ...
    fs.readFile(road, (err, data) => {
        ...
        if (err) {
            ...
            const dataByName = manipData.fromages.find(
...
exports.post = (request, response) => {
    ...
    fs.readFile(road, (err, data) => {
        ...
        if (err) {
            ...
            const arrayLength = existingData.fromages.length;
            // Si le tableau n'est pas vide               
            if (arrayLength > 1) {
                // Récupère le dernier objet du tableau fromages               
                const lastObject = existingData.fromages[arrayLength - 1];
                // Créer et insérer la new data avec l'id = à celle du dernier objet +1       
                existingData.fromages.push({
                    "id": lastObject.id + 1,
...
exports.modif = (request, response) => {
    ...
    fs.readFile(road, (err, data) => {
        if (err) {
            ...
            const data_id = existing_data.fromages.find(
                (obj) => obj.id === parseInt(request.params.id)
            );
...
exports.deleteDataById = (request, response) => {
    ...
    fs.readFile(road, (err, data) => {
        ...
        if (err) {
            ...
            const dataById = existingData.fromages.find(
                (obj) => obj.id === parseInt(request.params.id)
            );
            ...
            if (!dataById) {
                ...
                existingData.fromages = existingData.fromages.filter(
                    (obj) => obj.id != parseInt(request.params.id));
                fs.writeFile(road, JSON.stringify(existingData), (writeErr) => {
